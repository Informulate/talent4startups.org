...
class SessionsController extends Controller
{
	 ...
	 * @return
	 */
	public function store(SignIn $request)
	{
		...
		return Redirect::to('login')->with('error', 'Wrong email/password entered.');
	}
	...
	public function loginWithLinkedIn()
	{
		...
		if (!empty($code)) {
			...
			if (is_null($user) and $token) {
				if (is_null($type)) {
					...
					return view('sessions.select_type');
				}
		...
		if (Route::currentRouteName() === 'register_linked_in' and is_null($type)) {
			return view('registration.select_type');
		}
